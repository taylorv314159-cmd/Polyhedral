...
private lemma dual_auxGenSet' {C : PointedCone 𝕜 M} (hC : C.FG) :
  ...
  · rw [← Nonneg.mk_smul _ hu]
    exact Submodule.smul_mem _ _ (Submodule.subset_span rfl)
  simp only [← hs, dual_span, mem_dual, SetLike.mem_coe, map_sub, map_smul, smul_eq_mul, sub_nonneg]
  intro z hzS
  obtain hzw | hzw := lt_or_ge (p z w) 0
  · rw [← _root_.mul_le_mul_right_of_neg (inv_neg''.mpr hzw), mul_inv_cancel_right₀ hzw.ne]
    exact Finset.le_max' _ (p z v * (p z w)⁻¹) <|
...
--     dual p (dual p.flip {w} ∩ C) = span 𝕜 {w} ⊔ dual p C := by
--   simp [← auxGenSet_eq_dual_inf, ← dual_auxGenSet' hC]

variable {C : PointedCone 𝕜 M}

variable (p 𝕜) in
private lemma dual_inter_dual_singleton_dual_sup_span_singleton (hC : C.FG) (w : N) :
    dual p (C ∩ dual p.flip {w}) = dual p C ⊔ span 𝕜 {w} := by
  rw [inter_comm, sup_comm]
  simp_rw [← auxGenSet_eq_dual_inf, ← dual_auxGenSet' hC]

lemma dual_inf_dual_finite_dual_sup_finite (s : Finset N) :
    dual p (C ∩ dual p.flip s) = dual p C ⊔ span 𝕜 s := by classical
  induction s using Finset.induction with
  | empty => simp
  | insert w s hws hs =>
    rw [Finset.coe_insert, span_insert, dual_insert, ← Submodule.coe_inf]
    nth_rw 2 [sup_comm, inf_comm]
    rw [← sup_assoc, ← hs, ← inf_assoc]
    simpa using dual_inter_dual_singleton_dual_sup_span_singleton sorry w
...
variable (p 𝕜) in
private lemma foo {C : PointedCone 𝕜 M} {D : PointedCone 𝕜 N} (hC : C.FG) (hD : D.FG) :
    dual p (C ∩ dual p.flip D) = dual p C ⊔ D := by
  obtain ⟨s, rfl⟩ := hD
  induction s using Finset.induction with
  | empty => simp [hD]
  | insert w s hws hs =>
