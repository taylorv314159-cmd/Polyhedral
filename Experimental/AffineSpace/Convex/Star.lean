...
variable [AddTorsor V E] [AddTorsor W F]

section SMul

variable (𝕜) [SMul 𝕜 V] [SMul 𝕜 W] (x : E) (s : Set E)

/-- Star-convexity of sets. `s` is star-convex at `x` if every segment from `x` to a point in `s` is
contained in `s`. -/
def StarConvex (𝕜 : Type*) {V E : Type*} [Semiring 𝕜] [PartialOrder 𝕜]
    [AddCommGroup V] [AddTorsor V E] [SMul 𝕜 E] (x : E) (s : Set E) : Prop :=
  ∀ ⦃y : E⦄, y ∈ s → ∀ ⦃a b : 𝕜⦄, 0 ≤ a → 0 ≤ b → a + b = 1 → affineCombination ∈ s
